...
### Estrutura do Projeto (Layout Padrão Gradle)

```
project-root/
├── build.gradle.kts (ou build.gradle)
├── settings.gradle.kts (ou settings.gradle)
├── gradle.properties
├── CLAUDE.md
├── .claude/
│   └── commands/
...
### Configuração build.gradle.kts Essencial
...
dependencies {
    ...
    implementation("com.h2database:h2") // Para testes
